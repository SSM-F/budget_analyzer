...
class TestCsv:
    def test_populate_db_succesfully_populate_table(self,clean_table):
    ...
    def test_populate_db_contain_all_columns(self,clean_table):
        test_file = 'data/example_invoice.csv'
        test_table_name = 'expenses'
        populate_db(file_path=test_file, table_name=test_table_name)
    ...
    def test_parse_db_reads_all_content_of_file_and_adds_it_to_db(self,clean_table):
        test_file='data/invoice_today.csv'
        test_table= 'expenses'
        response = populate_db(file_path=test_file, table_name=test_table)
        rows = clean_table.run(f"SELECT * FROM {test_table}")
        assert response == rows
        assert len(response) == 2


    def test_populate_db_raise_exception(self):
        test_table_name = 'expenses'
        with pytest.raises(Exception) as e:
            populate_db(file_path='',table_name=test_table_name)
        assert "Wrong file type or path not found" in str(e)
    


    @patch('backend.parse.db_connection') 
    @patch('backend.parse.close_conn')  
    def test_populate_db_logging_info(self,mock_close, mock_db_conn, caplog):
        mock_conn = MagicMock()
...
class TestJson:
    def test_succesfully_populate_database_with_json_file(self,clean_table):
        test_file = 'data/invoice_holidays.json'
        test_table = 'expenses'
        response = populate_db(file_path=test_file, table_name=test_table)
        expected = clean_table.run(f"SELECT * FROM {test_table}")
        assert response == expected

        query = f"SELECT column_name FROM information_schema.columns WHERE table_name = '{test_table}'"
        columns_result = clean_table.run(query)
