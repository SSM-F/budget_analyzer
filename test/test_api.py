...
class TestPut:
    ...
    def test_put_endpoint_HTTPException(self,reset_db,test_client):
        test_invoice = 'not_existent_path.yml'
        fake_file = io.BytesIO(b"not a real json or csv")
        test_table = 'expenses' 
        response = test_client.put(f'/api/upload?table_name={test_table}',
                                   files={'file_path':(test_invoice,fake_file,'text/json')})
       
      
        assert response.status_code == 415
        assert response.json()['detail'] == 'Unsuported file'
...
class TestDelete:
    ...
    def test_endpoint_delete_raise_expection(self,reset_db,test_client):
        test_id = 200000
        test_table = 'expenses'
        response = test_client.delete(f'/api/delete/{test_table}/{test_id}')
        assert response.status_code == 404
        assert response.json()['detail'] == "Invoice with id number: 200000 not found. Please try different id number"
